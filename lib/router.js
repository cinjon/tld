...
Router.map( function() {
  ...
  this.route('editor', {
    ...
    onBeforeAction: function(pause) {
      var episode = Episodes.findOne({route:this.params.episode_route});
    ...
    waitOn: function() {
      episode = Episodes.findOne({route:this.params.episode_route});
      var show_route = this.params.route;
      return [
        Meteor.subscribe('episode_from_id', episode._id),
        Meteor.subscribe('show_from_route', show_route),
        Meteor.subscribe('highlights_from_episode', episode._id),
        Meteor.subscribe('people_names'),
        Meteor.subscribe('company_names'),
        Meteor.subscribe('people_from_episode', episode._id),
        Meteor.subscribe('chapters_from_episode', episode._id),
        Meteor.subscribe('companies_from_episode', episode._id)
      ];
    },
    data: function() {
      if (!this.ready()) {
  ...
  this.route('episodes_edit',{
    path: '/:show_route/:episode_route/edit',
    waitOn: function () {
      var episode = Episodes.findOne({route:this.params.episode_route});
      return Meteor.subscribe( 'episode_from_id', episode._id );
    },
    data: function () {
