...
Router.map( function() {
  ...
  this.route('viewer', {
    ...
    onBeforeAction: function(pause) { //Temporary until we open up View
      if (!SHOW_VIEW) {
        if (!Meteor.loggingIn() && Roles.userIsInRole(Meteor.userId(), 'editor')) { //have editors go to their queue
          go_to('queue', pause);
        } else if (!Meteor.loggingIn() && !Roles.userIsInRole(Meteor.userId(), 'admin')) { //have everyone not an admin go to sign in
          go_to(sign_in, pause);
        }
      }
    },
