...
Router.onBeforeAction(ir_before_hooks.redirect_to_sign_in, {only:['admin', 'editor', 'on_boarding']});
Router.onBeforeAction(ir_before_hooks.is_not_editor_or_admin, {only:['preview']});
...
Router.map( function() {
  ...
  this.route('editor_account', {
    ...
    waitOn: function() {
      return [
        ...
        Meteor.subscribe('payments_by_editor', Meteor.userId())
      ]
    }
  })
  ...
  this.route('payments_edit', {
    path: '/payments/:id/edit',
    waitOn: function () {
      return Meteor.subscribe( 'payment_from_id', this.params.id );
    },
    data: function () {
      return Payments.findOne({ _id: this.params.id });
    }
  });
