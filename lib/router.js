  ...
  this.route('editor', {
    ...
    onBeforeAction: function(pause) {
      ...
      Meteor.subscribe('episode_from_route', episode_route, function() {
        ...
        if (episode && episode.trial && !is_episode_editor_or_admin(Meteor.userId())) {
    ...
    onBeforeAction: function(pause) {
      if (!Meteor.loggingIn()) {
        if (!has_email(Meteor.user())) {
          go_to(on_boarding, pause);
        } else if (Meteor.subscribe('user_received_email', Meteor.userId()).ready() && !has_received_trial_email(Meteor.user())) {
...
var is_episode_editor_or_admin = function(user_id) {
  return episode.editor_id == user_id || (Meteor.subscribe('user_roles', user_id).ready() && Roles.userIsInRole(user_id, 'admin'));
}
