Meteor.methods({
  add_user_email: function(user_id, email) {
    ...
    if (!validate_email_for_exists(email)) {
      return 'exists';
    }
...
var validate_email_for_exists = function(user_id, email) {
  return Meteor.users.find({_id:{$ne:user_id}, emails:{$elemMatch:{address:email}}}).count() == 0;
}
