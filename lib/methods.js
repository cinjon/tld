Meteor.methods({
  ...
  remove_highlight: function(highlight_id) {
    ...
    var chapter = Chapters.findOne({_id:chapter_id});
    if (chapter.highlights.length > 1) {
  ...
  set_agree_to_terms: function(user_id) {
    Meteor.users.update({_id:user_id}, {$set:{signed_editor_legal:true}});
    //Create trial episodes for user as well.
    make_trial_episode(457, 'nerdist', user_id);
  },
  ...
  set_person_name: function(person_id, name) {
  ...
  set_person_twitter: function(person_id, twitter) {
    if (twitter == '') {
      return;
    }
  ...
  set_postedited_true: function(episode_id) {
    ...
    var validate_episode = validate_episode_for_completeness(episode);
    if (validate_episode['success']) {
...
var validate_episode_for_completeness = function(episode) {
  /*
     Do checks here for:
     - Unfinished links
     - People on highlights who aren't a host or guest
     - Untitled chapters
     - Unset summary
     - Unset title
     - A too long chapter
  */
  return {'success':false, 'message':"Still in Testing"}
}
