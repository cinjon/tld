Meteor.methods({
  ...
  add_highlight: function(info) {
    ...
    Chapters.find({episode_id:episode_id}, {sort:{start_time:1}}).forEach(function(chapter) {
      ...
      } else if (previous_chapter && chapter.start_time > start_time && start_time >= previous_chapter.start_time) { //previous chapter was correct one
  ...
  remove_highlight: function(highlight_id) {
    ...
    Episodes.update({_id:highlight.episode_id}, {$pull:{highlights:highlight_id}});
    Chapters.update({_id:highlight.chapter_id}, {$pull:{highlights:highlight_id}});
  },
  ...
  // set_introduction_chapter: function(episode_id) {
