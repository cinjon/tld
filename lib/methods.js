Meteor.methods({
  ...
  add_highlight: function(info) {
    ...
    chapters = Chapters.find({episode_id:episode_id}, {sort:{start_time:1}}).fetch();

    for (var i = 0; i < chapters.length; i++) {
      // handle case of single chapter or last chapter
      if (i == chapters.length - 1) {
        add_highlight_to_chapter(highlight_id, chapters[i], start_time);
      } else if (chapters[i].start_time <= start_time && chapters[i+1].start_time > start_time) {
        add_highlight_to_chapter(highlight_id, chapters[i], start_time);
        i = chapters.length + 1;
      }
    }
